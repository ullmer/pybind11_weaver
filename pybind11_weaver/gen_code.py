...
def gen_binding_codes(entities: Dict[str, entity_base.Entity], parent_sym: str, beg_id: int,
    ...
    while len(used_keys) != len(sorted_keys):
        for key in sorted_keys:
            ...
            print(f"Generating bindings for: {type(entity).__name__} ({getattr(entity, 'reference_name', lambda: 'unknown')()})")

            for d in entity.dependency():
                if d is None: print(f"Skipping null entity.dependency at {entity_obj_sym}"); continue
